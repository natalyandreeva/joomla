...
<table style="width:100%;" class="products-table">
...
<tr>
	...
	if ($parametri["show_name"] == 1) { ?>
		<th class="td-tovar"><?php echo JText::_('COM_VIRTUEMART_PRODUCTDETAILS_PRODUCT') ?></th>
	<?php $colspan += 1;}
	...
	if ($parametri["show_priceWithoutTax"] == 1 && $see_price) { ?>
		<th class="cell_priceWithoutTax"><?php echo JText::_('COM_VIRTUEMART_PRODUCT_PRICE_FORPACK') ?></th>
	<?php $colspan += 1;}
	if ($parametri["show_salesPrice"] == 1 && $see_price) { ?>
		<th class="cell_salesPrice"><?php echo JText::_('COM_VIRTUEMART_PRODUCT_PRICE_FORPACK') ?></th>
	<?php $colspan += 1;}
	...
	print_r($price_per_unit);
	  ...
	  */
	  ...
	  if (empty($product['child']['catproduct_inline_row'])) {		
		...
		// Product title
		if ($parametri["show_name"] == 1) {
			echo '<td class="td-tovar"><span class="product-title">'.$product['child']['product_name'].'</span>';
			echo '</td>';
		}
		...
		if ($parametri["show_priceWithoutTax"] == 1 && $see_price) {
			echo '<td class="cell_priceWithoutTax"><div class="product-price" id="priceWithoutTax_text_'.$i.'">'.$currency->createPriceDiv ('priceWithoutTax', '', $product['prices']).'</div></td>';
			//.round($product['prices']['priceWithoutTax'],2).' '.JText::_('CATPRODUCT_CURRENCY').'</td>';
		}
		// Product final price with tax
		if ($parametri["show_salesPrice"] == 1 && $see_price) {
			echo '<td class="cell_salesPrice"><div class="product-price" id="salesPrice_text_'.$i.'">'.$currency->createPriceDiv ('salesPrice', '', $product['prices']).'</div></td>';
...
<li>
		...
		<input type="hidden" name="virtuemart_product_id[]" value="<?php echo $product['child']['virtuemart_product_id'] ?>"/>
		</li>
		</ul>
		</form>
		</div>
